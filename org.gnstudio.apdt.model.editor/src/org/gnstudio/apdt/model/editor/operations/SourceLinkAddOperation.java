...
import org.gnstudio.apdt.model.editor.nodes.SourceLinkNode;
...
public class SourceLinkAddOperation extends AbstractNodeOperation {
	private SourceLink source;

	public SourceLinkAddOperation(ModelContext context, Object parent,
			NodeType nodeType, NodeMoveProvider moveProvider,
			Neighbor neighbor, boolean before) {
		super("Source-Link-Operation", context, parent, nodeType, moveProvider,
				neighbor, before);
	}
	...
	private AbstractNode<?> createNode() {
		...
		if (!(parent instanceof SourceLinkProvider))
			return null;

		final SourceLinkProvider provider = (SourceLinkProvider) parent;
		
		if(source==null)
			source = ModelFactory.eINSTANCE.createSourceLink();

		SourceLinkNode newNode = (new SourceLinkNode(refreshNode,
				source, new NodeDeleteProvider<SourceLink>() {

					public void delete(ModelContext context, SourceLink t) {
					...
					public AbstractOperation createDeleteOperation(
							ModelContext context, SourceLink t) {
						throw new AssertionError("Delete Operation not supported");
					}
				}));

		provider.setLink(source);
	...
	public IStatus undo(IProgressMonitor monitor, IAdaptable info)
		...
		if (newNode instanceof SourceLinkNode) {
			NodeDeleteProvider<SourceLink> deleteProvider = ((SourceLinkNode) newNode)
					.getDeleteProvider();
